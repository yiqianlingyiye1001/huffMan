...
#define MAX_FILE_SIZE (1024 * 1024 * 5)  // 最大支持5MB文件上传，可按需调整
...
// 文件处理核心函数 - 修复：删除全局缓冲区，改为堆内存申请+线程安全+兜底释放
...
// 解析form-data表单 - 不修改原始内存+安全拷贝文件名+无指针越界
int parseFormData(char *reqBody,int bodyLen,char *fileData,char *filename,int maxFileLen, char *boundary)
{
    ...
    if(filenamePos)
    {
        ...
        // 安全拷贝：计算长度+strncpy+手动加结束符，不修改原始请求内存，彻底解决内存越界
...
// 处理客户端请求 - 修复所有核心问题：recv超时+内存申请校验+动态内存释放+无内存泄漏
void handleClientRequest(SOCKET clientSock)
{
    ...
    // 修复核心错误：设置recv超时3秒，解决无限阻塞导致的30秒卡死问题
    ...
    if(newFileLen <= 0)
    {
        char *errResp = "HTTP/1.1 500 Internal Server Error\r\nAccess-Control-Allow-Origin: *\r\n\r\n文件加密处理失败";
        send(clientSock,errResp,strlen(errResp),0);
        free(totalBuf); free(fileData); free(processedBuf);
        closesocket(clientSock);
        return;
    }
